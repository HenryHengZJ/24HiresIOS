//
//  ShortlistedApplicantViewController.swift
//  JobIn24
//
...
class ShortlistedApplicantViewController: UIViewController,UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.applicantlist[indexPath.row].applicantImage == nil{
            applicantCell.applicantImage.image = UIImage(named: "userprofile_default")
        }else{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        nextViewController.status   = .hired
    ...
    override func viewDidLoad() {
        ...
        if(currentUser != nil){
            ...
            ref.child("UserPostedShortlistedApplicants").child(uid!).child(self.postkey).observe(.childRemoved, with: { (snapshot) in
                let ID = snapshot.key
            ...
            ref.child("UserPostedShortlistedApplicants").child(uid!).child(self.postkey).queryOrdered(byChild: "time").observe(.childAdded, with: { (snapshot) in
                
                if !snapshot.exists() {
                    print("no shortlisted")
                    self.removeLoadingScreen()
                    self.noApplicantsView.isHidden = false
                    return}
